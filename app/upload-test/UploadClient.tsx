...
interface ViewUrlResp {
  url: string;
}

// ====== Upload item state (single source of truth) ======
interface FileItem {
  id: string;
  file: File;
  key?: string;
...
export default function UploadClient() {
  // ---------- State ----------
  const [items, setItems] = useState<FileItem[]>([]); // ✅ state belongs inside component
    ...
    setItems((prev) => {
      ...
      const newcomers = results
        ...
        .map((r, idx) => ({
          ...
          file: new File([""], r.key.split("/").pop() || r.key, {
            type: "image/*",
          }),
          key: r.key,
          getUrl: r.url,
  ...
  async function clearGallery(prefix = DEFAULT_PREFIX): Promise<void> {
    if (
      !confirm(
        `This will permanently delete ALL objects under "${prefix}". Continue?`,
      )
    )
      return;
  ...
  // Undo-friendly delete flow (server presigns, S3 delete after 5s unless undone)
        ...
        setItems((prev) => [
          {
            ...
            progress: 100,
            status: "done",
            getUrl: prev.find((p) => p.id === id)?.getUrl,
          },
          ...prev,
        ]);
        alert(e.message);
  ...
  // Immediate delete entry point used by the UI (standardized signature)
  function deleteItemImmediate(item: FileItem) {
    // If not uploaded yet, just remove locally
    if (!item.key && !item.getUrl) {
      setItems((prev) => prev.filter((i) => i.id !== item.id));
      return;
    }

    // Derive key if missing (from presigned GET url path)
    let key = item.key;
    if (!key && item.getUrl) {
      try {
        key = new URL(item.getUrl).pathname.slice(1);
      } catch {}
    }
    if (!key) {
      console.warn("❌ Missing S3 key for delete");
      return;
    }

    // Use undo-friendly flow
    deleteItemByKey(item.id, key, item.file);
  }
  ...
  const startUploads = useCallback(async () => {
    ...
    const runNext = async (): Promise<void> => {
      ...
      setItems((prev) =>
        prev.map((i) =>
          i.id === next.id ? { ...i, status: "uploading", progress: 2 } : i,
        ),
      );
      ...
      try {
        ...
        setItems((prev) =>
          prev.map((i) => (i.id === next.id ? { ...i, putUrl: url, key } : i)),
        );

        await putWithProgress(url, next.file, (pct) => {
          setItems((prev) =>
            prev.map((i) => (i.id === next.id ? { ...i, progress: pct } : i)),
          );
        });
        ...
        setItems((prev) =>
          prev.map((i) =>
            i.id === next.id
              ? { ...i, getUrl, progress: 100, status: "done" }
              : i,
          ),
        );
      } catch (err: any) {
        setItems((prev) =>
          prev.map((i) =>
            i.id === next.id
              ? { ...i, status: "error", error: err?.message || String(err) }
              : i,
          ),
        );
      } finally {
  ...
  const imageGrid = useMemo(
    () => items.filter((i) => i.getUrl && isImage(i.file)),
    [items],
  );
  ...
  return (
    <div className="mx-auto max-w-5xl p-6 space-y-6">
      <h1 className="text-2xl font-semibold tracking-tight">
        S3 Upload Test · Thumbnails + Parallel Uploads
      </h1>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <button
          ...
          onClick={async () => {
            try {
              await clearGallery(DEFAULT_PREFIX);
            } catch (e: any) {
              alert(e.message);
            }
          }}
        ...
        <div className="ml-auto flex items-center gap-3">
          <div
            className="w-40 h-2 bg-gray-100 rounded-full overflow-hidden relative"
            title={`Overall ${overall}%`}
          >
            <div className="h-full bg-black" style={{ width: `${overall}%` }} />
          </div>
          <div className="text-sm text-gray-600 whitespace-nowrap">
            Overall: {overall}% · {done} done · {uploading} uploading · {queued}{" "}
            queued · Loaded from S3:{" "}
            {useMemo(
              () => items.filter((i) => i.id.startsWith("existing-")).length,
              [items],
            )}
          </div>
        </div>
          ...
          {items.map((item) => (
            ...
            >
              ...
              <div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden relative">
                ...
                />
                {item.status !== "done" && (
                  <span className="absolute right-1 -top-5 text-xs">
                    {item.progress}%
                  </span>
                )}
              </div>
              ...
              {item.status === "error" && (
                <div className="flex items-center gap-2">
                  <div className="text-xs text-red-600 truncate max-w-[240px]">
                    {item.error}
                  </div>
                  <button
              ...
              <div key={item.id} className="relative">
                {item.key && (
                  ...
                  >
                    ...
                    >
                      <path
                        d="M9 3h6m-9 4h12M9 7v12m6-12v12M5 7l1 14a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2l1-14"
                        stroke="currentColor"
                        strokeWidth="1.6"
                        strokeLinecap="round"
                ...
                <div
                  className="relative rounded-xl overflow-hidden border cursor-zoom-in"
          ...
          <ul className="space-y-2">
            {otherFiles.map((item) => (
              ...
              >
                ...
                <a href={item.getUrl!} target="_blank" className="underline">
                  Open
                </a>
              </li>
            ))}
