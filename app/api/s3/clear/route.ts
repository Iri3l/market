...
export async function POST(req: Request) {
  ...
  try {
    ...
    let token: string | undefined; let total = 0
    do {
      const list = await s3.send(new ListObjectsV2Command({
        Bucket: bucket, Prefix: dir, ContinuationToken: token, MaxKeys: 1000,
      }))
    ...
    return NextResponse.json({ error: err?.message || "clear failed" }, { status: code })
  }
}
